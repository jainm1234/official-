public class Decrypter implements RijndaelMethods {
	private int Nk, Nr;
	private byte[][] roundKey;

	private int[] subBox = {  };

	private int[] inv_subBox = {  };

	private int Rcon[] = {  };

	// Added by Malvika
	public byte[][] roundKey(byte[][] wordBox, byte[][] roundKey, int round) {
		byte[][] temp = new byte[wordBox.length][wordBox[0].length];

		for (int c = 0; c < 4; c++) {
			for (int l = 0; l < 4; l++)
				temp[l][c] = (byte) (wordBox[l][c] ^ roundKey[round * 4 + c][l]);
		}

		return temp;
	}

	@Override
	// Added by Malvika
	public byte[][] SubBytes(byte[][] wordBox) {
		for (int row = 0; row < 4; row++)
			for (int col = 0; col < 4; col++)
				wordBox[row][col] = (byte) (inv_subBox[(wordBox[row][col] & 0xff)]);

		return wordBox;
	}

	@Override
	// Added by Malvika
	public byte[][] ShiftRows(byte[][] wordBox) {
		byte[] t = new byte[4];
		for (int r = 1; r < 4; r++) {
			for (int c = 0; c < 4; c++)
				t[(c + r) % 4] = wordBox[r][c];
			for (int c = 0; c < 4; c++)
				wordBox[r][c] = t[c];
		}
		return wordBox;
	}

	@Override
	public byte[][] MixColumns(byte[][] s) {
		int[] sp = new int[4];
		byte b02 = (byte) 0x0e, b03 = (byte) 0x0b, b04 = (byte) 0x0d, b05 = (byte) 0x09;
		for (int c = 0; c < 4; c++) {
			sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c])
					^ FFMul(b04, s[2][c]) ^ FFMul(b05, s[3][c]);
			sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c])
					^ FFMul(b03, s[2][c]) ^ FFMul(b04, s[3][c]);
			sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c])
					^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
			sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c])
					^ FFMul(b05, s[2][c]) ^ FFMul(b02, s[3][c]);
			for (int i = 0; i < 4; i++)
				s[i][c] = (byte) (sp[i]);
		}

		return s;
	}

	// reference code
	public byte FFMul(byte a, byte b) {
		byte aa = a, bb = b, r = 0, t;
		while (aa != 0) {
			if ((aa & 1) != 0)
				r = (byte) (r ^ bb);
			t = (byte) (bb & 0x80);
			bb = (byte) (bb << 1);
			if (t != 0)
				bb = (byte) (bb ^ 0x1b);
			aa = (byte) ((aa & 0xff) >> 1);
		}
		return r;
	}

	// written by Malvika
	public byte[] decryptBloc(byte[] in) {
		byte[] temp = new byte[in.length];

		byte[][] wordBox = new byte[4][4];

		for (int i = 0; i < in.length; i++) {
			wordBox[i / 4][i % 4] = in[i % 4 * 4 + i / 4];
		}

		wordBox = roundKey(wordBox, roundKey, Nr);
		for (int round = Nr - 1; round >= 1; round--) {
			wordBox = SubBytes(wordBox);
			wordBox = ShiftRows(wordBox);
			wordBox = roundKey(wordBox, roundKey, round);
			wordBox = MixColumns(wordBox);

		}
		wordBox = SubBytes(wordBox);
		wordBox = ShiftRows(wordBox);
		wordBox = roundKey(wordBox, roundKey, 0);

		for (int i = 0; i < temp.length; i++)
			temp[i % 4 * 4 + i / 4] = wordBox[i / 4][i % 4];

		return temp;
	}

	// reference code to decrypt each 4 characters
	public byte[] decrypt(byte[] in, byte[] key) {
		int i;
		byte[] temp = new byte[in.length];
		byte[] bloc = new byte[16];

		Nk = key.length / 4;
		Nr = Nk + 6;
		roundKey = generateSubkeys(key);

		for (i = 0; i < in.length; i++) {
			if (i > 0 && i % 16 == 0) {
				bloc = decryptBloc(bloc);
				System.arraycopy(bloc, 0, temp, i - 16, bloc.length);
			}
			if (i < in.length)
				bloc[i % 16] = in[i];
		}
		bloc = decryptBloc(bloc);
		System.arraycopy(bloc, 0, temp, i - 16, bloc.length);

		temp = deletePadding(temp);

		return temp;
	}

	// reference code
	private byte[] deletePadding(byte[] input) {
		int count = 0;

		int i = input.length - 1;
		while (input[i] == 0) {
			count++;
			i--;
		}

		byte[] temp = new byte[input.length - count - 1];
		System.arraycopy(input, 0, temp, 0, temp.length);
		return temp;
	}

	// written by both with help from online sources
	private byte[][] generateSubkeys(byte[] key) {
		byte[][] change = new byte[4 * (Nr + 1)][4];

		int i = 0;
		while (i < Nk) {

			change[i][0] = key[i * 4];
			change[i][1] = key[i * 4 + 1];
			change[i][2] = key[i * 4 + 2];
			change[i][3] = key[i * 4 + 3];
			i++;
		}
		i = Nk;
		while (i < 4 * (Nr + 1)) {
			byte[] temp = new byte[4];
			for (int k = 0; k < 4; k++)
				temp[k] = change[i - 1][k];
			if (i % Nk == 0) {
				temp = SubWord(rotateWord(temp));
				temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
			} else if (Nk > 6 && i % Nk == 4) {
				temp = SubWord(temp);
			}
			change[i] = xor(change[i - Nk], temp);
			i++;
		}

		return change;
	}

	// for generateSubkeys()
	private byte[] rotateWord(byte[] input) {
		byte[] temp = new byte[input.length];
		temp[0] = input[1];
		temp[1] = input[2];
		temp[2] = input[3];
		temp[3] = input[0];

		return temp;
	}

	// Written by Jannie
	private byte[] SubWord(byte[] in) {
		byte[] temp = new byte[in.length];

		for (int i = 0; i < temp.length; i++)
			temp[i] = (byte) (subBox[in[i] & 0x000000ff] & 0xff);

		return temp;
	}

	// implemented by Jannie and Malvika
	private byte[] xor(byte[] first, byte[] second) {
		byte[] out = new byte[first.length];
		for (int i = 0; i < first.length; i++) {
			out[i] = (byte) (first[i] ^ second[i]);
		}
		return out;

	}

}
