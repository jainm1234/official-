public class Transmit implements RijndaelMethods {
	...
	private int[] subBox = { 0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5,
	...
	private int Rcon[] = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
	...
	public byte[] encryptBloc(byte[] in) {
		...
		byte[][] wordBox = new byte[4][4];

		for (int i = 0; i < in.length; i++)
			wordBox[i / 4][i % 4] = in[i % 4 * 4 + i / 4];

		wordBox = roundKey(wordBox, roundKey, 0);
		for (int round = 1; round < Nr; round++) {
			wordBox = SubBytes(wordBox);
			wordBox = ShiftRows(wordBox);
			wordBox = MixColumns(wordBox);
			wordBox = roundKey(wordBox, roundKey, round);
		}
		wordBox = SubBytes(wordBox);
		wordBox = ShiftRows(wordBox);
		wordBox = roundKey(wordBox, roundKey, Nr);

		for (int i = 0; i < temp.length; i++)
			temp[i % 4 * 4 + i / 4] = wordBox[i / 4][i % 4];

		return temp;
	}

	// reference code to encrypt each 4 characters
	public byte[] encrypt(byte[] in, byte[] key) {
		...
		int i;
		lens = 16 - in.length % 16;
		padding = new byte[lens];
		padding[0] = (byte) 0x80;

		for (i = 1; i < lens; i++)
			padding[i] = 0;

		byte[] temp = new byte[in.length + lens];
		byte[] bloc = new byte[16];

		roundKey = generateSubkeys(key);

		int count = 0;

		for (i = 0; i < in.length + lens; i++) {
			if (i > 0 && i % 16 == 0) {
	...
	// written by both with help from online sources
	private byte[][] generateSubkeys(byte[] key) {
		byte[][] change = new byte[4 * (Nr + 1)][4];

		int i = 0;
		while (i < Nk) {

			change[i][0] = key[i * 4];
			change[i][1] = key[i * 4 + 1];
			change[i][2] = key[i * 4 + 2];
			change[i][3] = key[i * 4 + 3];
			i++;
		}
		i = Nk;
		while (i < 4 * (Nr + 1)) {
	...
	private byte[] SubWord(byte[] in) {
		...
		for (int i = 0; i < temp.length; i++)
			temp[i] = (byte) (subBox[in[i] & 0x000000ff] & 0xff);
	...
	public byte[][] SubBytes(byte[][] words) {
		...
		for (int r = 0; r < words.length; r++) {
			for (int c = 0; c < words[0].length; c++) {
				temp[r][c] = (byte) (subBox[(words[r][c] & 0xff)] & 0xff);
			}
		}
		return temp;
	}
