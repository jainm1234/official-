
public class Transmit implements RijndaelMethods {

	private int keycounter;

	private static int[] subBox = {  };

	public Transmit() {
		keycounter = 0;

	}

	private byte[] xor(byte[] first, byte[] second) {
		byte[] out = new byte[first.length];
		for (int i = 0; i < first.length; i++) {
			out[i] = (byte) (first[i] ^ second[i]);
		}
		return out;

	}

	public byte[][] RoundKey(byte[][] words) {

		// TODO Auto-generated method stub

		return null;

	}

	public byte[][] SubBytes(byte[][] words) {

		// TODO Auto-generated method stub

		byte[][] temp = new byte[words.length][words[0].length];
		for (int r = 0; r < words.length; r++) {
			for (int c = 0; c < words[0].length; c++) {
				temp[r][c] = (byte) (subBox[(words[r][c] & 0x000000ff)] & 0xff);
			}
		}
		return temp;
	}

	public byte[] SubByteArray(byte[] words) {

		// TODO Auto-generated method stub

		byte[] temp = new byte[words.length];
		for (int r = 0; r < words.length; r++) {
			temp[r] = (byte) (subBox[(words[r] & 0x000000ff)] & 0xff);
		}

		return temp;
	}

	public byte[][] ShiftRows(byte[][] words) {

		// TODO Auto-generated method stub
		byte[] byteRow = new byte[4];
		for (int r = 1; r < 4; r++) {
			for (int c = 0; c < 4; c++)
				byteRow[c] = words[r][(c + r) % 4];
			for (int c = 0; c < 4; c++)
				words[r][c] = byteRow[c];
		}

		return words;

	}

	public byte[][] MixColumns(byte[][] words) {

		// TODO Auto-generated method stub

		return null;

	}

	public byte[][] KeyExpansion(byte[][] words, byte[] roundconstant) {
		byte[] wordslastcol = new byte[words.length];
		for (int i = 0; i < words.length; i++) {
			wordslastcol[i] = words[i][3];

		}
		byte top = wordslastcol[0];
		for (int i = 1; i < 3; i++) {
			wordslastcol[i - 1] = wordslastcol[i];
		}
		wordslastcol[3] = top;

		byte[] subwordslastcol = SubByteArray(wordslastcol);

		byte[] newfirstcol = xor(subwordslastcol, roundconstant);

		byte[] thatcol = new byte[4];

		for (int c = 0; c < 4; c++) {
			for (int r = 0; r < 4; r++) {
				thatcol[r] = words[r][c];

			}

		}

		return null;

	}

}